        ...
        Observable<String> observable = Observable.create(new Observable.OnSubscribe<String>() {
            ...
            public void call(Subscriber<? super String> subscriber) {
                if (!subscriber.isUnsubscribed()) {
                    subscriber.onNext(tweet1);
                }

                subscriber.onNext(tweet2);
                subscriber.onNext(tweet3);
                subscriber.onNext(tweet4);
                subscriber.onNext(tweet5);
            }
        })
                .observeOn(Schedulers.io());

        observable
                .filter(tweet -> tweet.contains("created_at"))
                .map(tweet -> gson.fromJson(tweet, Tweet.class))
                .filter(Tweet::isValidTweet)
                .subscribe(tweet -> System.out.println("tweet = " + tweet), Throwable::printStackTrace);
